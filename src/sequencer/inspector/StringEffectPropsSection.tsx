...
import {
  ...
  stringEffectDefinitionsArray,
} from "../../common/effects/stringEffectDefinitions";
import { PropEditor } from "./props/PropEditor";
import type { z } from "zod";
import { useState, useEffect, useRef, startTransition } from "react";
import { EffectSimulation } from "./simulation/EffectSimulation";
import type { AllTrackEventModels } from "../../../shared/models/sequencer";

type StringEffectPropsSectionProps = {
  event: AllTrackEventModels;
};

export function StringEffectPropsSection({
  event,
}: StringEffectPropsSectionProps) {
  ...
  useEffect(() => {
    ...
    const effect =
      event.kind === "string_effect" ? event.effectDefinitionId : null;
  ...
  const primaryEffectId =
    event.kind === "string_effect" ? event.effectDefinitionId : null;
  const effectDefinition = Object.values(stringEffectDefinitions).find(
  ...
  return (
    <Stack gap="md">
      <Select
        label="Effect"
        value={event.kind === "string_effect" ? event.effectDefinitionId : null}
        onChange={(effectId) => {
          if (!effectId) return;
          try {
        ...
        data={stringEffectDefinitionsArray.map((def) => ({
          value: def.id,
      ...
      {effectDefinition && (
        <EffectSimulation
          EffectComponent={effectDefinition.component}
          props={displayParams}
        />
      )}
    </Stack>
